...
const Payment : NextPage = () => {
    ...
    return (
        <Layout>
            {data?.currentUser?.packingorder.length && data?.currentUser?.packingorder?.map((box) => <div key={box.id} onClick={() => setPackageDetail({...packageDetail, show: !packageDetail.show, id:box.id})}>
                {box.id}</div>)}
