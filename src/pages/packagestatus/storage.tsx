...
import { Package, PackageStatus, Address } from "@prisma/client";
...
const Storage : NextPage = () => {
    ...
    const currentChoosenAddress = addressList?.find(address => address.id === currentAddressId )
    ...
    return (
        <>
        {addressPage && <AddAddress setAddressList={setAddressList} addressList={addressList} setAddressPage={setAddressPage} currentUserId={data?.currentUser?.id as string} setCurrentAddressId={setCurrentAddressId}/>}
        ...
        <Layout>
            ...
            <div className="flex w-[100%] text-[1rem] justify-between items-center px-4 py-2" onClick={() => setAddressPage(prev => !prev)}>
               ...
               <div className="text-xs ">{currentChoosenAddress && 
                 <div className="flex gap-1 items center">
                     <p>{currentChoosenAddress.firstName}</p> 
                     <p>{currentChoosenAddress.secondName}</p>
                     <p>{currentChoosenAddress.country}</p>
                </div>
               }</div>
